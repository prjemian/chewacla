...
@pytest.mark.parametrize(
    "symbol,expected,ctx",
    [
        pytest.param("x+", np.array([1.0, 0.0, 0.0]), does_not_raise(), id="symbol-plus"),
        pytest.param("+x", np.array([1.0, 0.0, 0.0]), does_not_raise(), id="plus-symbol"),
        pytest.param("x-", np.array([-1.0, 0.0, 0.0]), does_not_raise(), id="symbol-minus"),
        pytest.param("-x", np.array([-1.0, 0.0, 0.0]), does_not_raise(), id="minus-symbol"),
        pytest.param(" y+ ", np.array([0.0, 1.0, 0.0]), does_not_raise(), id="surrounding-whitespace"),
        pytest.param("+Y", np.array([0.0, 1.0, 0.0]), does_not_raise(), id="uppercase"),
        pytest.param("z+", np.array([0.0, 0.0, 1.0]), does_not_raise(), id="z-axis"),
        pytest.param("xx", None, pytest.raises(ValueError), id="two-letters"),
        pytest.param("++", None, pytest.raises(ValueError), id="two-symbbols"),
        pytest.param("x", None, pytest.raises(ValueError), id="no-symbol"),
        pytest.param("", None, pytest.raises(ValueError), id="empty"),
        pytest.param(" xyz", None, pytest.raises(ValueError), id="three-letters-no-symbols"),
        pytest.param("x++", None, pytest.raises(ValueError), id="letter-and-two-symbols"),
        pytest.param("1+", None, pytest.raises(ValueError), id="notalpha-plus"),
        pytest.param("+1", None, pytest.raises(ValueError), id="plus-notalpha"),
        pytest.param("++x", None, pytest.raises(ValueError), id="plus-plus-notalpha"),
        pytest.param(
            "b+",
            None,
            pytest.raises(
